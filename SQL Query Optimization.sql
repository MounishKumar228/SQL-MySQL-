/*
Always check the EXEC PLAN to confirm performance improvements when optimizing the query
...
INDEX Idx_Orders_OrderStatus ON Sales.Orders(OrderStatus);
...
-- functions on columns affects the indexes -> update -> affects performance
...
-- Leading wild cards prevents the usage of INDEXES but TRAILS will not prevent
...
CREATE -- NONCLUSTERED (SQL SERVER)
INDEX IX_Orders_CustomerID ON Sales.Orders(CustomerID);
...
-- Bad Practice
-- Correlated Subquery
-- SQL execute aggregations for every  row
SELECT 
    c.CustomerID, 
    c.FirstName,
...
-- INTO Sales.SalesSummary (SQL server) in MySQL use CREATE TABLE table_name as (select...)
FROM Sales.Orders
GROUP BY MONTH(OrderDate);

-- if we update columns then we have to drop the table and then again run it
